...
5. Do NOT include any markdown block (like ```json) in the response text itself since structured JSON format is requested.";
        ...
        try {
            $response = Illuminate\Support\Facades\Http::timeout(120)->post("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={$apiKey}", [
                'contents' => [
                    [
                        'parts' => [
                            [
